#!/usr/bin/env bash
# ---------------------------------------------------------------------------
#  Ahmed’s Linux bootstrap – modular, fault-tolerant, **now interactive** 🎉
# ---------------------------------------------------------------------------
...
#  • Interactive CLI: -h help  -l list modules  -i pick modules via menu.
#    You can still pass module names directly (e.g. `./setup.sh install_neovim`).
# ---------------------------------------------------------------------------
...
HOME_DIR=""
SUDO=""
RAU() { :; }  # placeholder; real impl injected in detect_user
...
Options:
  ...
  -i            Interactive mode – pick modules from a menu

If you pass module names positionally, only those run (in order).
If no arguments are supplied the script performs a full run.
EOF
}
...
interactive_menu() {
  ...
  if [[ $chosen_numbers == *"all"* ]]; then
    ...
    for n in $chosen_numbers; do
      n="${n//[!0-9]}"  # strip non-digits
      (( n>=1 && n<=${#ALL_MODULES[@]} )) && SELECTED_MODULES+=("${ALL_MODULES[n-1]}")
    done
  fi
  [[ ${#SELECTED_MODULES[@]} == 0 ]] && { warn "Nothing selected"; exit 1; }
}
...
install_prereqs() {
  $SUDO apt update -y
  local pkgs=(git openssh-client curl wget gnupg ca-certificates lsb-release xclip xsel unzip whiptail)
...
install_toolchain() {
  ...
  local pkgs=(
    ...
    zsh git ripgrep tmux thefuck delta git-delta pyenv-virtualenv
    build-essential cmake gcc fontconfig jq curl wget gnupg ca-certificates
    eza
  )
  $SUDO apt install -y "${pkgs[@]}" || warn "Some APT packages failed – continuing"

  # --- eza fallback logic (unchanged) ---
...
##############################################################################
### 5. Clone dotfiles (SSH → HTTPS fallback)                              ####
##############################################################################
clone_dotfiles() {
...
create_symlinks() {
  ...
  # extras
  link "$proj/config/home/oh-my-zsh"        "$HOME_DIR/.oh-my-zsh"
...
##############################################################################
### 7. Z-shell, Oh-My-Zsh, Powerlevel10k                                 ####
##############################################################################
setup_zsh() {
  $SUDO chsh -s "$(command -v zsh)" "$TARGET_USER" || true

  if [[ ! -d "$HOME_DIR/.oh-my-zsh" ]]; then
    RAU "zsh -c \"sh -c '\$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)' '' --unattended\""
  fi
...
install_pyenv_virtualenv() {
  ...
  # also try APT package (won't exist on all distros but harmless)
  $SUDO apt install -y pyenv-virtualenv || true
}
...
##############################################################################
### 13. Sync Neovim plugins (Lazy.nvim)                                   ####
##############################################################################
sync_nvim_plugins() {
...
##############################################################################
### 15. Reload shell (source .zshrc)                                     ####
##############################################################################
reload_shell() {
  RAU "source '$HOME_DIR/.zshrc'" || true
}
...
run_step "Pyenv / virtualenv"         install_pyenv_virtualenv
run_step "Lazygit"                    install_lazygit
run_step "Tmux plugins"               setup_tmux_plugins
...
run_step "Reload shell"               reload_shell
